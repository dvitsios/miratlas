...
curInputFile = args[1]
outputProfilesDir <- args[2]
mir_to_filter <- args[3]



dataset_prefix = curInputFile
dataset_prefix = gsub("\\-mod_counts\\.txt","", dataset_prefix)
...
trim <- function (x) gsub("^\\s+|\\s+$", "", x)
    ...
    global_profile_file = paste(outputProfilesDir, paste(dataset_prefix, ".global_mods_profile_data.csv", sep=""), sep="/")
